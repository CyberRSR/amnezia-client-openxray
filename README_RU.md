...
- Android-протокол WWG объединяет два профиля AmneziaWG v2 без создания двух системных VPN-интерфейсов: входной туннель работает в Go netstack, а выходной владеет единственным Android VPN-интерфейсом.
- При импорте WWG сначала выбирается входной `.conf`, затем выходной `.conf`. Оба файла должны иметь версию протокола AmneziaWG 2 и полный набор параметров v2.
- Сериализованный монитор постоянно проверяет relay и раз в пять секунд выполняет пробу через Android VPN network. Два неуспешных раунда запускают полное переподключение без ограничения количества попыток.
- Portable export/import `.vpn` сохраняет оба слоя AWG v2. Приватные профили, ключи и данные доступа к серверам не включаются в исходники и релизные файлы.
