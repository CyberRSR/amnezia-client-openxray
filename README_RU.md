...
## Дополнения ветки WWG

- Android-протокол WWG объединяет два согласованных профиля AmneziaWG (v2/v2 или v3/v3): входной туннель работает в Go netstack, а выходной владеет единственным Android VPN-интерфейсом. В AWG3 поле `protocol_version` остаётся равным `2`, режим определяется по новым полям v3.
- DNS и обычный трафик проходят через оба слоя. Сериализованный монитор проверяет VPN каждые пять секунд, не разрывает соединение при частичном отказе одного endpoint и после двух полностью неуспешных раундов использует ограниченный backoff с jitter.
- Экспорт управляемого WWG-профиля в `.vpn` или QR создаёт независимые ключи, PSK, адреса `/32` и peer на обоих серверах; профиль-донор продолжает работать одновременно. Старый профиль без ограниченной provisioning-capability подключается как раньше, но перед безопасным экспортом его нужно переиздать.
- Экспортированный профиль содержит приватные данные подключения и должен храниться как секрет. Рабочие профили, ключи, capability-токены и серверные данные не включаются в исходники и публичные релизы.
- Подробности: [архитектура и формат профиля WWG](docs/WWG_RU.md).
